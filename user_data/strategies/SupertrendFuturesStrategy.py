...
class SupertrendFuturesStrategy(IStrategy):
    ...
    rsi_oversold = IntParameter(15, 40, default=25, space="sell")
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # 3. 趋势强度 > 0.5% (过滤震荡市)
        conditions.append(dataframe['trend_strength'] > 0.5)
        
        # 4. SSL Channel 看涨
        conditions.append(dataframe['close'] > dataframe['ssl_up'])
        
        # 5. 成交量确认 (高于平均20%)
        conditions.append(dataframe['volume'] > dataframe['volume'].rolling(20).mean() * 1.2)
